...
export async function POST(request: NextRequest) {
  ...
  if (positionId) {
    try {
      ...
      const id = typeof positionId === "string" ? positionId : String(positionId);
      const updated = await Position.findByIdAndUpdate(id, { $set: { jd_text: parsedText } }, { new: true, runValidators: true });
      if (!updated) {
        console.error("No document found with the given positionId:", id);
      } else {
    ...
    console.error("No positionId provided");
  }
